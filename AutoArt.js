(async () => {
  const CONFIG = {
    ...
    START_Y: 1148,
    PIXELS_PER_LINE: 100,
    DELAY: 30000, // Увеличенная задержка между запросами
    RETRY_DELAY: 60000, // Задержка при ошибках
  ...
  const paintPixel = async (x, y) => {
    if (state.retryCount >= CONFIG.MAX_RETRIES) {
      updateUI(state.language === 'pt' ? '❌ Muitas tentativas falhas. Parando...' : '❌ Too many failed attempts. Stopping...', 'error');
      state.running = false;
      toggleButtonState();
      return null;
    }
    ...
    const result = await fetchAPI(`https://backend.wplace.live/s0/pixel/${CONFIG.START_X + x}/${CONFIG.START_Y + y}`, {
      method: 'POST',
